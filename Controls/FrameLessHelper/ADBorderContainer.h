#ifndef ADBORDERCONTAINER_H
#define ADBORDERCONTAINER_H
...
class ADBorderContainer : public QObject
{
    Q_OBJECT
public:
    ADBorderContainer(QWidget *parent, uint16_t minWindowHeight = 800, uint16_t minWindowWidth = 800, uint16_t borderSize = 5);
...
private:
    ...
    // 内部边框类，防止外部创建
    class ADBorder : public QLabel
    {
    public:
        // 设置边框属性
        ADBorder(QWidget *parent, BorderType type, ADBorderContainer *outClass);
    ...
    private:
        ADBorderContainer *container; // 保存外部类指针
        BorderType MyType;
